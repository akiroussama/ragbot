...
exports.PgVectorAdapter = void 0;
const pg_1 = require("pg");
const common_1 = require("@nestjs/common");
class PgVectorAdapter {
    name = 'pgvector';
    logger = new common_1.Logger(PgVectorAdapter.name);
    pool;
    config;
    schema;
    constructor(config) {
    ...
    async delete(collectionName, ids, options = {}) {
        ...
        const client = await this.pool.connect();
        try {
            await client.query(`DELETE FROM ${this.schema}.${tableName} WHERE id = ANY($1)`, [ids]);
    ...
    async getCollectionInfo(name) {
      ...
      `, [this.schema, tableName]);
            ...
            // Get row count
            const countResult = await client.query(`SELECT COUNT(*) as count FROM ${this.schema}.${tableName}`);
            // Get table size
...
exports.PgVectorAdapter = PgVectorAdapter;
